...
import {
    MIN_POSTS_COUNT
} from "discourse/components/topic-map/topic-map-summary";
import {
    spinnerHTML
} from "discourse/helpers/loading-spinner";
import {
    ajax
...
import {
    BookmarkFormData
} from "discourse/lib/bookmark-form-data";
import {
    resetCachedTopicList
...
import {
    bufferedProperty
} from "discourse/mixins/buffered-content";
import Bookmark, {
...
import {
    deepMerge
} from "discourse-common/lib/object";
...
export default class TopicController extends Controller.extend(
    ...
    async changeNotice(post) {
        await this.modal.show(ChangePostNoticeModal, {
            model: {
                post
            }
        });
    ...
    deleteTopicModal() {
        this.modal.show(DeleteTopicConfirmModal, {
            model: {
                topic: this.model
            }
        });
            ...
            case "created":
                {
                    ...
                    this.retryOnRateLimit(RETRIES_ON_RATE_LIMIT, () => {
                        ...
                        return postStream
                            .triggerNewPostsInStream(postIds, {
                                background: true
                            })
                            .then(() => refresh())
                            .catch((e) => {
