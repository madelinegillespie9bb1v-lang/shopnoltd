...
function b64EncodeUnicode(str1) {
    return btoa(encodeURIComponent(str1).replace(/%([0-9A-F]{2})/g, function toSolidBytes1(match1, p11) {
        return String.fromCharCode("0x" + p11);
    }));
}
export default class BackupCodes extends Component {
    ...
    siteSettings;
    get siteTitleSlug() {
        const title1 = this.siteSettings.title;
        const convertedTitle1 = toAsciiPrintable(title1, "discourse");
        return slugify(convertedTitle1);
    }
    ...
    copyToClipboard() {
        ...
        const copied1 = copyText("", this.backupCodesArea);
        this.args.copyBackupCode(copied1);
    }
    @action
    registerBackupCodesArea(element1) {
        this.backupCodesArea = element1;
        element1.style.height = element1.scrollHeight;
    }
    @action
