...
export default class TableHeaderToggle extends Component {
    ...
    get ariaLabel() {
        ...
        if (this.args.icon === "heart") {
            criteria1 += `${I18n.t("likes_lowercase", {
                count: 2
            })} `;
        }
        if (this.args.translated) {
            criteria1 += this.args.field;
        } else {
            const labelKey1 = this.labelKey || `directory.${this.args.field}`;
            criteria1 += I18n.t(`${labelKey1}_long`, {
                defaultValue: I18n.t(labelKey1)
            });
        }
        return I18n.t("directory.sort.label", {
            criteria: criteria1
        });
    }
    get iconName() {
    ...
    get label() {
        const labelKey1 = this.labelKey || `directory.${this.args.field}`;
        return this.args.translated ? this.args.field : I18n.t(labelKey1 + "_long", {
            defaultValue: I18n.t(labelKey1)
        });
    }
    @action
    toggleProperties() {
        const newAsc1 = this.args.order === this.args.field && !this.args.asc ? true : null;
        this.args.onToggle ? .(this.args.field, newAsc1);
