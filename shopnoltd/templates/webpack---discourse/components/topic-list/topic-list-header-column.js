...
export default class TopicListHeaderColumn extends Component {
    ...
    bulkSelectAll() {
        ...
        document.querySelectorAll("input.bulk-select:not(:checked)").forEach((el1) => el1.click());
    }
    @action
    bulkClearAll() {
        this.args.bulkSelectHelper.autoAddTopicsToBulkSelect = false;
        document.querySelectorAll("input.bulk-select:checked").forEach((el1) => el1.click());
    }
    @action
    onClick() {
    ...
    @action
    onKeyDown(event1) {
        if (event1.key === "Enter" || event1.key === " ") {
            this.args.changeSort(this.args.order);
