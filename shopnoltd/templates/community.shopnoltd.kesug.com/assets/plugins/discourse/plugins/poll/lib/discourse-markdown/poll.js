define("discourse/plugins/poll/lib/discourse-markdown/poll", ["exports", "discourse-i18n"], function(_exports, _discourseI18n) {
    ...
    const ALLOWED_ATTRIBUTES = ["chartType", "close", "groups", "max", "min", "name", "order", "public", "results", "status", "step", "type"];
    ...
    function addPollContainer(state, titleTokens, pollTokens) {
        ...
        for (let i = 0; i < pollTokens.length; i++) {
            if (pollTokens[i].type === "list_item_open") {
                let listItemCloseIndex = pollTokens.findIndex((t, j) => j > i && t.type === "list_item_close");
                if (listItemCloseIndex === -1) {
                    continue;
                }
                let text = pollTokens.slice(i, listItemCloseIndex + 1).filter(c => c.type === "text" || c.type === "inline").map(c => c.content).join(" ");
    ...
    function setup(helper) {
        helper.allowList(["a.button.cast-votes", "a.button.toggle-results", "div.poll-buttons", "div.poll-container", "div.poll-info_counts-count", "div.poll-info_counts", "div.poll-info", "div.poll-title", "div.poll", "div[data-*]", "li[data-*]", "span.info-label", "span.info-number", "span.info-text"]);
