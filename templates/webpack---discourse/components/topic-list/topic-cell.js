...
export default class TopicCell extends Component {
    ...
    messageBus;
    get newDotText() {
        return this.currentUser ? .trust_level > 0 ? "" : I18n.t("filters.new.lower_title");
    }
    get participantGroups() {
        if (!this.args.topic.get("participant_groups")) {
            return [];
        }
        return this.args.topic.get("participant_groups").map((name1) => ({
            name: name1,
            url: groupPath(name1)
        }));
    }
    ...
    @action
    onTitleBlur(event1) {
        event1.target.classList.remove("selected");
    }
    static {
        template(`
