...
export default class SoftwareUpdatePrompt extends Component {
    ...
    @bind
    onAsset(version1) {
        if (this.session.assetVersion !== version1) {
            this.session.requiresRefresh = true;
        }
        if (!this.timeoutHandler && this.session.requiresRefresh) {
            if (isTesting()) {
                ...
                this.timeoutHandler = discourseLater(() => this.updatePromptState(true), 1000 * 60 * 24 * 60);
            }
        }
    }
    updatePromptState(value1) {
        ...
        const firstProp1 = value1 ? "showPrompt" : "animatePrompt";
        const secondProp1 = value1 ? "animatePrompt" : "showPrompt";
        this[firstProp1] = value1;
        if (isTesting()) {
            this[secondProp1] = value1;
        } else {
            discourseLater(() => (this[secondProp1] = value1), 500);
        }
    }
    @action
    refreshPage() {
