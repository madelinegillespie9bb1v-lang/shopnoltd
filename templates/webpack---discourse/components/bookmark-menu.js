...
export default class BookmarkMenu extends Component {
    ...
    setReminderShortcuts() {
        ...
        const custom1 = this.timeShortcuts.custom();
        custom1.label = "time_shortcut.more_options";
        this.reminderAtOptions.push(custom1);
    }
    get existingBookmark() {
        return this.bookmarkManager.trackedBookmark ? .id ? this.bookmarkManager.trackedBookmark : null;
    }
    ...
    get buttonClasses() {
        let cssClasses1 = [
            "bookmark widget-button bookmark-menu__trigger"
        ];
        if (!this.args.showLabel) {
            cssClasses1.push("btn-icon no-text");
        } else {
            cssClasses1.push("btn-icon-text");
        }
        if (this.args.buttonClasses) {
            cssClasses1.push(this.args.buttonClasses);
        }
        if (this.existingBookmark) {
            cssClasses1.push("bookmarked");
            if (this.existingBookmark.reminderAt) {
                cssClasses1.push("with-reminder");
            }
        }
        return cssClasses1.join(" ");
    }
    get buttonIcon() {
        if (this.existingBookmark ? .reminderAt) {
    ...
    @action
    reminderShortcutTimeTitle(option1) {
        if (!option1.time) {
            return "";
        }
        return option1.time.format(I18n.t(option1.timeFormatKey));
    }
    @action
    onBookmark() {
        this.bookmarkCreatePromise = this.bookmarkManager.create();
        this.bookmarkCreatePromise.then(() => {
            ...
            this.toasts.success({
                ...
                data: {
                    message: I18n.t("bookmarks.bookmarked_success")
                }
            });
        }).catch((error1) => popupAjaxError(error1)).finally(() => {
    ...
    async onRemoveBookmark() {
        try {
            const response1 = await this.bookmarkManager.delete();
            this.bookmarkManager.afterDelete(response1, this.existingBookmark.id);
            this.toasts.success({
                duration: 1500,
                data: {
                    icon: "trash-can",
    ...
    @action
    async onChooseReminderOption(option1) {
        if (this.bookmarkCreatePromise) {
            await this.bookmarkCreatePromise;
        }
        if (option1.id === TIME_SHORTCUT_TYPES.CUSTOM) {
            this._openBookmarkModal();
        } else {
            this.existingBookmark.selectedReminderType = option1.id;
            this.existingBookmark.selectedDatetime = option1.time;
            this.existingBookmark.reminderAt = option1.time;
            try {
                ...
                this.toasts.success({
                    ...
                    data: {
                        message: I18n.t("bookmarks.reminder_set_success")
                    }
                });
            } catch (error1) {
                popupAjaxError(error1);
    ...
    async _openBookmarkModal() {
        this.dMenu.close();
        try {
            const closeData1 = await this.modal.show(BookmarkModal, {
                model: {
                    bookmark: this.existingBookmark,
                    afterSave: (savedData1) => {
                        return this.bookmarkManager.afterSave(savedData1);
                    },
                    afterDelete: (response1, bookmarkId1) => {
                        this.bookmarkManager.afterDelete(response1, bookmarkId1);
                    }
                }
            });
            this.bookmarkManager.afterModalClose(closeData1);
        } catch (error1) {
            popupAjaxError(error1);
        }
    }
