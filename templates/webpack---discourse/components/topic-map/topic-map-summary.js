...
export default class TopicMapSummary extends Component {
    ...
    get shouldShowParticipants() {
        return (this.args.topic.posts_count >= MIN_POSTS_COUNT && this.args.topicDetails.participants ? .length >= MIN_USERS_COUNT_FOR_AVATARS && !this.site.mobileView);
    }
    get first5Participants() {
        return this.args.topicDetails.participants.slice(0, MIN_PARTICIPANTS_COUNT);
    }
    get readTimeMinutes() {
        const calculatedTime1 = Math.ceil(Math.max(this.args.topic.word_count / this.siteSettings.read_time_word_count, (this.args.topic.posts_count * MIN_POST_READ_TIME_MINUTES) / 60));
        return calculatedTime1 > MIN_READ_TIME_MINUTES ? calculatedTime1 : null;
    }
    ...
    get topRepliesLabel() {
        return this.topRepliesSummaryEnabled ? I18n.t("summary.show_all_label") : I18n.t("summary.short_label");
    }
    get loneStat() {
    ...
    get hasLikes() {
        return (this.args.topic.like_count > MIN_LIKES_COUNT && this.args.topic.posts_count > MIN_POSTS_COUNT);
    }
    get hasUsers() {
    ...
    @action
    postUrl(post1) {
        return this.args.topic.urlForPostNumber(post1.post_number);
    }
    @action
    fetchMostLiked() {
        const cacheKey1 = `top3LikedPosts_${this.args.topic.id}`;
        const cachedData1 = this.mapCache.get(cacheKey1);
        this.loading = true;
        if (cachedData1) {
            this.top3LikedPosts = cachedData1;
            this.loading = false;
            return;
        }
        const filter1 = `/search.json?q=" " topic%3A${this.args.topic.id} order%3Alikes`;
        ajax(filter1).then((data1) => {
            const top3LikedPosts1 = data1.posts.filter((post1) => post1.post_number > 1 && post1.like_count > 0).sort((a1, b1) => b1.like_count - a1.like_count).slice(0, 3);
            this.mapCache.set(cacheKey1, top3LikedPosts1);
            this.top3LikedPosts = top3LikedPosts1;
        }).catch((error1) => {
            this.dialog.alert(I18n.t("generic_error_with_reason", {
                error: `http: ${error1.status} - ${error1.body}`
            }));
        }).finally(() => {
            this.loading = false;
        });
    ...
    @action
    fetchViews() {
        const cacheKey1 = `topicViews_${this.args.topic.id}`;
        const cachedData1 = this.mapCache.get(cacheKey1);
        this.loading = true;
        if (cachedData1) {
            this.views = cachedData1;
            this.loading = false;
            return;
        }
        ajax(`/t/${this.args.topic.id}/view-stats.json`).then((data1) => {
            if (data1.stats.length) {
                this.views = data1;
            } else {
                data1.stats.push({
            ...
            this.mapCache.set(cacheKey1, data1);
        }).catch((error1) => {
            this.dialog.alert(I18n.t("generic_error_with_reason", {
                error: `http: ${error1.status} - ${error1.body}`
            }));
