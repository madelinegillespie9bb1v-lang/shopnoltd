...
export function addAboutPageActivity(name1, func1) {
    pluginActivitiesFuncs.push({
        name: name1,
        func: func1
    });
}
...
export default class AboutPage extends Component {
    ...
    get siteActivities() {
        ...
        return list1.concat(this.siteActivitiesFromPlugins());
    }
    get contactInfo() {
    ...
    get siteAgeString() {
        const creationDate1 = new Date(this.args.model.site_creation_date);
        let diff1 = new Date() - creationDate1;
        diff1 /= 1000 * 3600 * 24 * 30;
        if (diff1 < 1) {
            return I18n.t("about.site_age.less_than_one_month");
        } else if (diff1 < 12) {
            return I18n.t("about.site_age.month", {
                count: Math.round(diff1)
            });
        } else {
            diff1 /= 12;
            return I18n.t("about.site_age.year", {
                count: Math.round(diff1)
            });
        }
    }
    ...
    siteActivitiesFromPlugins() {
        const stats1 = this.args.model.stats;
        const statKeys1 = Object.keys(stats1);
        const configs1 = [];
        for (const {
                name: name1,
                func: func1
            } of pluginActivitiesFuncs) {
            let present1 = false;
            const periods1 = {};
            for (const stat1 of statKeys1) {
                const prefix1 = `${name1}_`;
                if (stat1.startsWith(prefix1)) {
                    present1 = true;
                    const period1 = stat1.replace(prefix1, "");
                    periods1[period1] = stats1[stat1];
                }
            }
            if (!present1) {
                continue;
            }
            const config1 = func1(periods1);
            if (config1) {
