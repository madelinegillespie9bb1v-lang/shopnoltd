...
} from "discourse/components/about-page";
import {
    addBulkDropdownButton
} from "discourse/components/bulk-select-topics-dropdown";
import {
    addCardClickListenerSelector
} from "discourse/components/card-contents-base";
import {
...
} from "discourse/components/d-document";
import {
    addToolbarCallback
...
import {
    forceDropdownForMenuPanels as glimmerForceDropdownForMenuPanels
} from "discourse/components/glimmer-site-header";
import {
    addGlobalNotice
} from "discourse/components/global-notice";
import {
    headerButtonsDAG
} from "discourse/components/header";
import {
    headerIconsDAG
} from "discourse/components/header/icons";
import {
    addWidgetCleanCallback
} from "discourse/components/mount-widget";
import {
    addPluginOutletDecorator
} from "discourse/components/plugin-connector";
import {
...
import {
    addSearchSuggestion
} from "discourse/components/search-menu/results/assistant";
import {
    addItemSelectCallback as addSearchMenuAssistantSelectCallback
} from "discourse/components/search-menu/results/assistant-item";
import {
...
import {
    addOnKeyUpCallback
} from "discourse/components/search-menu/search-term";
import {
    REFRESH_COUNTS_APP_EVENT_NAME as REFRESH_USER_SIDEBAR_CATEGORIES_SECTION_COUNTS_APP_EVENT_NAME
} from "discourse/components/sidebar/user/categories-section";
import {
    addTopicParticipantClassesCallback
} from "discourse/components/topic-map/topic-participant";
import {
    setDesktopScrollAreaHeight
} from "discourse/components/topic-timeline/container";
import {
    addTopicTitleDecorator
} from "discourse/components/topic-title";
import {
    setNotificationsLimit as setUserMenuNotificationsLimit
} from "discourse/components/user-menu/notifications-list";
import {
    addUserMenuProfileTabItem
} from "discourse/components/user-menu/profile-tab-content";
import {
    addLegacyStat as addLegacyAboutPageStat
} from "discourse/controllers/about";
import {
    addDiscoveryQueryParam
...
} from "discourse/controllers/user-notifications";
import {
    registerCustomUserNavMessagesDropdownRow
} from "discourse/controllers/user-private-messages";
import {
    addExtraIconRenderer,
    replaceCategoryLinkRenderer
} from "discourse/helpers/category-link";
import {
    addUsernameSelectorDecorator
} from "discourse/helpers/decorate-username-selector";
import {
    registerCustomAvatarHelper
} from "discourse/helpers/user-avatar";
import {
    addBeforeAuthCompleteCallback
} from "discourse/instance-initializers/auth-complete";
import {
...
} from "discourse/lib/admin-plugin-header-actions";
import classPrepend, {
...
} from "discourse/lib/register-topic-footer-dropdown";
import {
    replaceTagRenderer
} from "discourse/lib/render-tag";
import {
    addTagsHtmlCallback
} from "discourse/lib/render-tags";
import {
    addFeaturedLinkMetaDecorator
} from "discourse/lib/render-topic-featured-link";
import {
...
import {
    addAdminSidebarSectionLink
} from "discourse/lib/sidebar/admin-sidebar";
import {
    addSectionLink as addCustomCommunitySectionLink
...
} from "discourse/lib/sidebar/user/tags-section/base-tag-section-link";
import {
    consolePrefix
} from "discourse/lib/source-identifier";
import {
    includeAttributes
} from "discourse/lib/transform-post";
import {
    _addTransformerName,
...
import {
    addCustomUserFieldValidationCallback
} from "discourse/mixins/user-fields-validation";
import Composer, {
...
} from "discourse/routes/new-category";
import {
    setNotificationsLimit
} from "discourse/routes/user-notifications";
import {
...
import {
    addDecorator
} from "discourse/widgets/post-cooked";
import {
    addButton,
    apiExtraButtons,
    removeButton,
...
} from "discourse/widgets/poster-name";
import {
    changeSetting,
    createWidget,
    decorateWidget,
...
import {
    addImageWrapperButton
} from "discourse-markdown-it/features/image-controls";
import {
    CUSTOM_USER_SEARCH_OPTIONS
} from "select-kit/components/user-chooser";
import {
    modifySelectKit
...
function canModify(klass, type, resolverName, changes) {
    ...
    if (!changes.pluginId) {
        ...
        console.warn(consolePrefix(), "To prevent errors in tests, add a `pluginId` key to your `modifyClass` call. This will ensure the modification is only applied once.");
        return true;
    }
...
let PluginApi = class PluginApi {
    ...
    _resolveClass(resolverName, opts) {
        ...
        if (this.container.cache[normalized] || (normalized === "model:user" && this.container.lookup("service:current-user"))) {
            // eslint-disable-next-line no-console
            console.error(consolePrefix(), `Attempted to modify "${resolverName}", but it was already initialized earlier in the boot process (e.g. via a lookup()). Remove that lookup, or move the modifyClass call earlier in the boot process for changes to take effect. https://meta.discourse.org/t/262064`);
            return;
        }
        ...
        if (!klass) {
            if (!opts.ignoreMissing) {
                // eslint-disable-next-line no-console
                console.warn(consolePrefix(), `"${normalized}" was not found by modifyClass`);
            }
            return;
        }
        return klass;
    ...
    registerBehaviorTransformer(transformerName, behaviorCallback) {
        _registerTransformer(transformerName, transformerTypes.BEHAVIOR, behaviorCallback);
    }
    /**
    ...
    registerValueTransformer(transformerName, valueCallback) {
        _registerTransformer(transformerName, transformerTypes.VALUE, valueCallback);
    }
    /**
    ...
    attachWidgetAction(widget, actionName, fn) {
        const widgetClass = queryRegistry(widget) || this.container.factoryFor(`widget:${widget}`) ? .class;
        if (!widgetClass) {
            // eslint-disable-next-line no-console
            console.error(consolePrefix(), `attachWidgetAction: Could not find widget ${widget} in registry`);
            return;
        }
    ...
    addPostAdminMenuButton(callback) {
        this.container.lookup("service:admin-post-menu-buttons").addButton(callback);
    }
    /**
    ...
    addTopicAdminMenuButton(callback) {
        this.container.lookup("service:admin-topic-menu-buttons").addButton(callback);
    }
    /**
    ...
    addToolbarPopupMenuOptionsCallback(opts) {
        deprecated("`addToolbarPopupMenuOptionsCallback` has been renamed to `addComposerToolbarPopupMenuOption`", {
    ...
    addFlagProperty() {
        deprecated("addFlagProperty has been removed. Use the reviewable API instead.", {
    ...
    addHeaderPanel() {
        // eslint-disable-next-line no-console
        console.error(consolePrefix(), `api.addHeaderPanel: This API was decommissioned. Use api.headerIcons instead.`);
    }
    /**
     ...
     */
    addUserMenuGlyph() {
        deprecated("addUserMenuGlyph has been removed. Use api.registerUserMenuTab instead.", {
    ...
    addNavigationBarItem(item) {
        if (!item["name"]) {
            // eslint-disable-next-line no-console
            console.warn(consolePrefix(), "A 'name' is required when adding a Navigation Bar Item.", item);
        } else {
    ...
    decoratePluginOutlet(outletName, callback, opts) {
        deprecated("decoratePluginOutlet is deprecated because modifying an Ember-rendered DOM tree can lead to very unexpected errors. Use CSS or plugin outlet connectors instead", {
    ...
    decorateTopicTitle(callback) {
        deprecated("decorateTopicTitle is deprecated because modifying an Ember-rendered DOM tree can lead to very unexpected errors. Use plugin outlet connectors instead", {
    ...
    addToHeaderIcons(icon) {
        // eslint-disable-next-line no-console
        console.error(consolePrefix(), `api.addToHeaderIcons: This API was decommissioned. Use api.headerIcons instead.`);
    }
    /**
    ...
    registerHomeLogoHrefCallback(callback) {
        _registerTransformer("home-logo-href", transformerTypes.VALUE, ({
            value
        }) => callback(value));
    }
    /**
    ...
    refreshUserSidebarCategoriesSectionCounts() {
        const appEvents = this._lookupContainer("service:app-events");
        appEvents ? .trigger(REFRESH_USER_SIDEBAR_CATEGORIES_SECTION_COUNTS_APP_EVENT_NAME);
    }
    /**
    ...
    includeUserFieldPropertyOnSave(userFieldProperty) {
        this.container.lookup("service:admin-custom-user-fields").addProperty(userFieldProperty);
    }
    /**
    ...
    addAdminPluginConfigurationNav(pluginId, mode, links) {
        ...
        if (!validModes.includes(mode)) {
            // eslint-disable-next-line no-console
            console.warn(consolePrefix(), `${mode} is an invalid mode for admin plugin config pages, only ${validModes} are usable.`);
            return;
        }
