...
// ----------------------
// 🗂️ DOWNLOAD ALL FILES AS ZIP
// ----------------------
if (isset($_POST['downloadFiles'])) {
    $zipFile = "site_backup_" . date("Y-m-d_H-i-s") . ".zip";
    ...
    if ($zip->open($zipFile, ZipArchive::CREATE | ZipArchive::OVERWRITE)) {
        ...
        foreach ($files as $file) {
            if (!$file->isDir()) {
                ...
                $relative = substr($filePath, strlen($rootPath) + 1);
...
// ----------------------
// 📥 UPLOAD & REPLACE FILES FROM ZIP
// ----------------------
if (isset($_POST['uploadZip']) && !empty($_FILES['zipFile']['tmp_name'])) {
    $zip = new ZipArchive;
    if ($zip->open($_FILES['zipFile']['tmp_name']) === TRUE) {
        $zip->extractTo(__DIR__); // extract into current directory
        $zip->close();
        $_SESSION['flashSuccess'] = "✅ Files uploaded and replaced successfully.";
    } else {
...
<div class="container">
    ...
    <div class="card p-3 mb-3">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                <form method="post">
                    <button type="submit" name="downloadDB" class="btn btn-primary w-100">⬇️ Download Full Database (.sql)</button>
                </form>
            </div>
            <div class="col-md-6">
    ...
    <div class="card p-3">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                <form method="post">
                    <button type="submit" name="downloadFiles" class="btn btn-primary w-100">⬇️ Download All Files (.zip)</button>
                </form>
            </div>
            <div class="col-md-6">
                <form method="post" enctype="multipart/form-data">
