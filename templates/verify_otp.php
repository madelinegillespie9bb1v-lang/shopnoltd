...
if ($current_time > $otp_end_time) {
    $pdo->prepare("UPDATE users SET generated_otp_number=NULL, generated_otp_time=NULL, generated_otp_end_time=NULL WHERE id=?")
        ->execute([$userId]);
    unset($_SESSION['pending_user']);
    header('Location: login.php');
    exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($entered_otp === (string)$otp_generated) {
        // ✅ OTP correct
        $role = strtolower(trim($dbUser['user_role'] ?? 'user'));

        // Set verified user session
        $_SESSION['user'] = [
            'id' => $dbUser['id'],
        ...
        // Verify file exists
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . $redirectUrl)) {
            $redirectUrl = '/login.php'; // fallback
        }
        ...
        $pdo->prepare("UPDATE users SET generated_otp_number=NULL, generated_otp_time=NULL, generated_otp_end_time=NULL WHERE id=?")
            ->execute([$userId]);
        unset($_SESSION['pending_user']);
        $flashMessage = "❌ Invalid OTP. Please login again.";
        header("Location: login.php");
        exit;
    }
}
...
<div class="container">
<h2>Verify OTP</h2>
<p>Enter the 6-digit OTP sent to your Browser.</p>

<?php if ($flashMessage): ?>
<p style="color:red; font-weight:bold;"><?php echo htmlspecialchars($flashMessage); ?></p>
<?php endif; ?>
...
Your OTP for testing: <strong id="otpValue"><?php echo htmlspecialchars($otp_generated); ?></strong>
...
function updateTimer() {
    ...
    if (remaining <= 0) {
        timerElem.textContent = "⏰ OTP expired. Please login again.";
    } else {
