...
$referral = isset($_GET['ref']) ? trim($_GET['ref']) : '';
...
// Get numeric code for preselection
$selectedCode = '+1'; 
...
.container{ max-width:480px; margin:50px auto; padding:25px; border:1px solid #ddd; border-radius:10px; background:#fff; }
...
.actions button{width:100%;padding:12px;background:#28a745;color:#fff;border:none;border-radius:5px;font-size:15px;cursor:pointer;}
...
<div class="container">
    ...
    <form action="register.php" method="post" class="registration-form" autocomplete="off">
        <input type="hidden" name="csrfmiddlewaretoken" value="<?= $csrf_token ?>">

        <?php if(!empty($referral)): ?>
        	<input type="hidden" name="referrer" value="<?= htmlspecialchars($_GET['ref'] ?? '') ?>">
        <?php endif; ?>
        ...
        <input type="tel" name="phone" id="phoneInput" required placeholder="Enter number">
        ...
        <div class="small-link"><a href="/login.php">Already have an account? Login</a></div>
    </form>
</div>
...
document.querySelectorAll('.togglePassword').forEach(btn => {
  btn.addEventListener('touchstart', togglePass);
