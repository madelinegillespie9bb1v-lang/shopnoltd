...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- Approve or Reject Fund/Transfer/Withdraw ---
    if (isset($_POST['approve']) || isset($_POST['reject'])) {
        $reqId = (int)($_POST['req_id'] ?? 0);
        $newStatus = isset($_POST['approve']) ? 'approved' : 'rejected';

        // Detect which table request belongs to
        $tablesToCheck = ['fund_requests', 'withdraw_requests'];
        foreach ($tablesToCheck as $tbl) {
            $stmt = $pdo->prepare("SELECT id FROM $tbl WHERE id = ?");
            $stmt->execute([$reqId]);
            if ($stmt->fetch()) {
                $update = $pdo->prepare("UPDATE $tbl SET status = ?, reviewed_at = NOW() WHERE id = ?");
                $update->execute([$newStatus, $reqId]);
                $_SESSION['flashSuccess'] = ucfirst($tbl).' request '.$newStatus.' successfully!';
                break;
            }
        }
    ...
    if (isset($_POST['admin_adjust_balance'])) {
        $uid    = (int)($_POST['adjust_user'] ?? 0);
        ...
        if ($uid > 0 && $amount != 0) {
            ...
            $pdo->prepare("INSERT INTO transactions (user_id, amount, note, type, created_at) VALUES (?, ?, ?, ?, NOW())")
                ->execute([$uid, $amount, $note, 'admin_adjust']);
            $pdo->commit();
            $_SESSION['flashSuccess'] = "Balance adjusted successfully.";
        } else {
            $_SESSION['flashError'] = "Invalid balance adjustment request.";
        }
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit;
    }
    ...
    // --- Change Role ---
    if (isset($_POST['admin_change_role'])) {
        $uid = (int)($_POST['adjust_user'] ?? 0);
        $newRole = trim($_POST['change_role'] ?? 'user');
        $pdo->prepare("UPDATE users SET user_role = ? WHERE id = ?")->execute([$newRole, $uid]);
        $_SESSION['flashSuccess'] = "User role updated to $newRole.";
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit;
    }
    ...
    if (isset($_POST['admin_change_status'])) {
        $uid = (int)($_POST['adjust_user'] ?? 0);
        $newStatus = trim($_POST['change_status'] ?? 'active');
        $pdo->prepare("UPDATE users SET account_status = ? WHERE id = ?")->execute([$newStatus, $uid]);
        $_SESSION['flashSuccess'] = "User account status changed to $newStatus.";
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit;
    }

    // --- Upload CSV / XLSX ---
    if (isset($_POST['uploadBtn']) && !empty($_FILES['uploadFile']['tmp_name'])) {
        $tmp = $_FILES['uploadFile']['tmp_name'];
        $ext = strtolower(pathinfo($_FILES['uploadFile']['name'], PATHINFO_EXTENSION));

        if (in_array($ext, ['csv', 'xlsx'])) {
            $_SESSION['flashSuccess'] = "File uploaded successfully (parsed logic pending).";
        } else {
            $_SESSION['flashError'] = "Invalid file format.";
        }
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit;
    }
    ...
    if (isset($_POST['downloadCSV'])) {
        ...
        fputcsv($out, ['ID','Username','Email','Role','Status','Balance','Created']);
        $rows = $pdo->query("SELECT id, username, email, user_role, account_status, balance, created_at FROM users")->fetchAll(PDO::FETCH_ASSOC);
...
// Navigation items
$nav = [
    ['title'=>"Backup Dashboard",   'icon'=>'bi-cash',            'color'=>'primary',   'link'=>'/backup_manager.php',       'count'=>null,                      'role'=>'admin', 'desc'=>"Backup All."],
    ...
    ['title'=>"Transactions",       'icon'=>'bi-receipt',         'color'=>'success',   'link'=>'#transactionsTable',   'count'=>null,                      'role'=>'admin', 'desc'=>"View all transactions."],
    ['title'=>"Fund Requests",      'icon'=>'bi-wallet2',         'color'=>'warning',   'link'=>'#fundRequestsTable',   'count'=>null,                      'role'=>'admin', 'desc'=>"Track all fund requests or transfers."],
    ['title'=>"Withdraw Requests",  'icon'=>'bi-cash',            'color'=>'danger',    'link'=>'#withdrawRequestsTable','count'=>null,                     'role'=>'admin', 'desc'=>"View your pending or approved withdraw requests."],
    ...
    ['title'=>"Manage Jobs Post and Done Task",'icon'=>'bi-gear','color'=>'secondary', 'link'=>'/admin_jobs.php',       'count'=>$totalJobsCount ?? 0,      'role'=>'admin', 'desc'=>"Task Approval Section."],
    ...
    ['title'=>"Contact or Support", 'icon'=>'bi-people',          'color'=>'info',      'link'=>'/contact.php',          'count'=>$userCount ?? 0,           'role'=>'admin', 'desc'=>"Reach admin/support for any account or job-related issues."],
    ['title'=>"Do Job / Start Work",'icon'=>'bi-play-circle',     'color'=>'primary',   'link'=>'/job_work.php',         'count'=>$openJobsCount ?? 0,       'role'=>'admin', 'desc'=>"Find available jobs and start working."],
    ['title'=>"My Work List",       'icon'=>'bi-clipboard-check', 'color'=>'success',   'link'=>'/my_work_list.php',     'count'=>$userDoneJobs ?? 0,        'role'=>'admin', 'desc'=>"View your current and completed tasks."],
...
<div class="card mb-4">
  <?php
// Fetch users list
$usersList = $pdo->query("SELECT * FROM users")->fetchAll(PDO::FETCH_ASSOC);
$usersListCount = count($usersList); // proper count
?>
<div class="card-header bg-danger text-white">
    <h5>Users Overview (<?= $usersListCount ?>)</h5>
</div>

  <div class="card-body table-wrap">
    <table id="usersOverviewTable" class="table table-sm table-bordered">
      <thead>
        <tr>
          <th>ID</th><th>Username</th><th>Email</th><th>Role</th><th>Status</th><th>Balance</th><th>Created</th><th>Admin Actions</th>
        </tr>
      </thead>
      <tbody>
      <?php foreach($usersList as $u): ?>
        <tr>
          ...
          <td><?= safeFormat($u['balance']) ?></td>
          <td><?= htmlspecialchars($u['created_at'] ?? '') ?></td>
          <td>
            <form method="post" class="d-inline" style="width:300px">
              <input type="hidden" name="adjust_user" value="<?= $u['id'] ?>">
              <div class="input-group input-group-sm mb-1">
                <input type="number" step="0.01" name="adjust_amount" class="form-control" placeholder="+/- Balance">
                <input type="text" name="adjust_note" class="form-control" placeholder="Note">
                <button class="btn btn-outline-primary" name="admin_adjust_balance" value="1">Apply</button>
              </div>
              <div class="input-group input-group-sm mb-1">
                <select name="change_role" class="form-select">
                  <?php $roles = ['user','admin','moderator']; foreach($roles as $role): ?>
                  <option value="<?= $role ?>" <?= ($u['user_role']==$role)?'selected':'' ?>><?= ucfirst($role) ?></option>
                  <?php endforeach; ?>
                </select>
                <button class="btn btn-outline-success" name="admin_change_role" value="1">Change Role</button>
              </div>
              <div class="input-group input-group-sm">
                <select name="change_status" class="form-select">
                  <?php $statuses = ['active','inactive','suspended']; foreach($statuses as $status): ?>
                  <option value="<?= $status ?>" <?= ($u['account_status']==$status)?'selected':'' ?>><?= ucfirst($status) ?></option>
                  <?php endforeach; ?>
                </select>
                <button class="btn btn-outline-warning" name="admin_change_status" value="1">Change Status</button>
              </div>
            </form>
          </td>
        </tr>
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
<style>.nav-card:hover{transform:translateY(-3px); box-shadow:0 5px 12px rgba(0,0,0,0.2); text-decoration:none;}</style>
</head>
<body class="p-3 bg-light">


<div class="card p-3 mb-4">
<h5>Upload / Download Users Data</h5>
<div class="row g-3">
<div class="col-md-6">
<form method="post" enctype="multipart/form-data">
<label class="form-label">Upload CSV/XLSX:</label>
<input type="file" name="uploadFile" accept=".csv,.xlsx" class="form-control mb-2" required>
<button type="submit" name="uploadBtn" class="btn btn-success btn-sm">Upload</button>
</form>
</div>
<div class="col-md-6">
<form method="post">
<label class="form-label">Download Data:</label><br>
<button type="submit" name="downloadCSV" class="btn btn-primary btn-sm">Download CSV</button>
</form>
</div>
</div>
</div>
