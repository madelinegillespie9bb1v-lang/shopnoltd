...
export default class SectionFormLink extends Component {
    ...
    dragCssClass;
    dragCount = 0;
    isAboveElement(event1) {
        event1.preventDefault();
        const target1 = event1.currentTarget;
        const domRect1 = target1.getBoundingClientRect();
        return event1.offsetY < domRect1.height / 2;
    }
    @action
    dragHasStarted(event1) {
        event1.dataTransfer.effectAllowed = "move";
    ...
    dropItem(event1) {
        ...
        this.dragCount = 0;
        this.args.reorderCallback(this.args.link, this.isAboveElement(event1));
        this.dragCssClass = null;
    }
    @action
    dragEnd() {
