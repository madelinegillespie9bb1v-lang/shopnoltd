...
export default class UsernamePreference extends Component {
    ...
    editing = false;
    @tracked
    newUsername = this.args.user.username;
    @tracked
    errorMessage = null;
    @tracked
    saving = false;
    ...
    @action
    async onInput(event1) {
        this.newUsername = event1.target.value;
        this.taken = false;
        this.errorMessage = null;
        ...
        const result1 = await User.checkUsername(this.newUsername, undefined, this.args.user.id);
        if (result1.errors) {
            this.errorMessage = result1.errors.join(" ");
